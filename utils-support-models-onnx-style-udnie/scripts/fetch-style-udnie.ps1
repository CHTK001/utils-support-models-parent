# fetch-style-udnie.ps1
# 下载 Fast Neural Style Transfer - Udnie（抽象）ONNX 模型
# 模型大小约 6.6MB
...
if (Test-Path $OutFile) {
    Write-Host "已存在: $ModelName [Udnie 抽象]" -ForegroundColor Green
    exit 0
}

Write-Host "下载: $ModelName [Udnie 抽象]..." -ForegroundColor Cyan
try {
